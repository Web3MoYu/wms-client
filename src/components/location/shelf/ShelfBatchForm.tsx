...
// 批量添加的货架项接口
interface BatchShelfItem
  extends Omit<Shelf, 'id' | 'createTime' | 'updateTime'> {
  key: string;
  codeValid?: boolean;
  nameValid?: boolean;
  codeError?: string;
...
// 批量添加提交数据接口
type BatchShelfData = Pick<
  Shelf,
  'areaId' | 'shelfName' | 'shelfCode' | 'status'
>;
        ...
        batchShelfList.map((item) => {
          if (item.key === key) {
            ...
            if (
              ...
              updatedItem.codeValid =
                field === 'shelfCode' ? undefined : item.codeValid;
              updatedItem.nameValid =
                field === 'shelfName' ? undefined : item.nameValid;
              updatedItem.codeError =
                field === 'shelfCode' ? undefined : item.codeError;
              updatedItem.nameError =
                field === 'shelfName' ? undefined : item.nameError;
            }
            return updatedItem;
          }
          return item;
        ...
        render: (_: any, record: BatchShelfItem) => (
          <div>
            <Input
              ...
              onChange={(e) =>
                updateBatchItem(record.key, 'shelfName', e.target.value)
              }
        ...
        render: (_: any, record: BatchShelfItem) => (
          <div>
            <Input
              ...
              onChange={(e) =>
                updateBatchItem(record.key, 'shelfCode', e.target.value)
              }
