...
import { Table, Button, Input, Select, Space, message, Typography, Alert } from 'antd';
...
import { Storage, checkStorageExists } from '../../../api/location-service/StorageController';
...
// 批量添加的库位项接口
interface BatchStorageItem extends Omit<Storage, 'id' | 'createTime' | 'updateTime'> {
  key: string;
  codeValid?: boolean;
  codeError?: string;
...
// 批量添加提交数据接口
type BatchStorageData = Pick<Storage, 'areaId' | 'shelfId' | 'locationCode' | 'locationName' | 'status'>;
...
const StorageBatchForm = forwardRef<BatchFormRef, StorageBatchFormProps>(({
  ...
  const [batchStorageList, setBatchStorageList] = useState<BatchStorageItem[]>([]);
  ...
  // 获取货架列表
  const fetchShelfList = useCallback(async (areaId: string) => {
    if (!areaId) return;
  ...
  const addBatchRow = () => {
    ...
    setBatchStorageList([
      ...batchStorageList,
      {
        ...
        productId: '',
        codeValid: true,
        areaCode: lastAreaId ? areaCodes[lastAreaId] : '',
        shelfCode: lastShelfId ? shelfCodes[lastShelfId] : '',
      },
    ]);
  };
  ...
  const updateBatchItem = (key: string, field: string, value: any) => {
    ...
    if (field === 'locationCode') {
      ...
      if (currentItem && currentItem.areaId && currentItem.shelfId && value) {
        validateLocationCode(key, value, currentItem.shelfId, currentItem.areaId);
      }
    }
  };

  // 校验库位编码
  const validateLocationCode = async (key: string, code: string, shelfId: string, areaId: string) => {
  ...
  // 更新项目的验证状态
  const updateItemValidation = (key: string, isValid: boolean, errorMsg?: string) => {
  ...
  const validateAllItems = async (): Promise<boolean> => {
    ...
    try {
      ...
      const validationPromises = batchStorageList.map(item => 
        validateLocationCode(item.key, item.locationCode, item.shelfId, item.areaId)
      );

      await Promise.all(validationPromises);

      // 检查是否全部校验通过
      const allValid = batchStorageList.every(item => item.codeValid !== false);
  ...
  const handleBatchSubmit = async () => {
    try {
      ...
      // 准备提交数据
      const storageData: BatchStorageData[] = batchStorageList.map(item => ({
        areaId: item.areaId,
  ...
  // 模糊搜索产品
  const handleProductSearch = debounce(async (key: string, productName: string) => {
  ...
  const columns = [
    ...
    {
      title: <Space><span style={{ color: '#ff4d4f' }}>*</span>所属区域</Space>,
      dataIndex: 'areaId',
      key: 'areaId',
      width: 180,
    ...
    {
      title: <Space><span style={{ color: '#ff4d4f' }}>*</span>所属货架</Space>,
      dataIndex: 'shelfId',
      key: 'shelfId',
      width: 180,
    ...
    {
      title: <Space><span style={{ color: '#ff4d4f' }}>*</span>库位编码</Space>,
      ...
      render: (code: string, record: BatchStorageItem) => (
        <Input
          ...
          onChange={(e) => updateBatchItem(record.key, 'locationCode', e.target.value)}
          addonBefore={
    ...
    },
    {
      ...
      render: (productId: string, record: BatchStorageItem) => (
        <Select
          ...
          onChange={(value) => updateBatchItem(record.key, 'productId', value)}
        >
