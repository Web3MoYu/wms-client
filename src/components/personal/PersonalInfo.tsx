...
const PersonalInfo: React.FC = observer(() => {
  ...
  // 处理头像上传
  const handleAvatarChange = useCallback((info: UploadChangeParam<UploadFile>) => {
  ...
  const renderInfoDescriptions = useCallback(() => (
    <>
      ...
      >
        ...
        <Descriptions.Item 
          label={<Space><PhoneOutlined /> 手机号码</Space>}
        >
          {user.user?.phone || '未设置'}
        </Descriptions.Item>
        <Descriptions.Item 
          label={<Space><WechatOutlined /> 微信</Space>}
        >
  ...
  ), [user.user, handleEdit, showPasswordModal]);
  ...
  const renderEditForm = useCallback(() => (
    ...
    >
      ...
      <Form.Item 
        label={<Space><WechatOutlined /> 微信绑定</Space>}
        style={styles.formSection}
      >
        <Input
    ...
    <div style={styles.avatarContainer}>
      ...
      >
        <div style={styles.avatarWrapper}>
          ...
          />
          {editing && (
            <CameraOutlined style={styles.avatarUploadIcon} />
          )}
        </div>
      </Upload>
  ...
  ), [avatar, editing, handleAvatarChange, beforeUpload, user]);
