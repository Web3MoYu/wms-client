...
const RoleManager: React.FC = () => {
  ...
  // 显示编辑角色模态框
  const showEditModal = useCallback((role: Role) => {
    setEditingRole(role);
    setIsModalVisible(true);
  }, []);
  ...
  // 处理删除角色
  const handleDelete = useCallback((roleId: string, roleName: string) => {
    Modal.confirm({
      title: '确认删除',
  ...
  // 处理角色新增或更新
  const handleAddOrUpdate = useCallback(async (role: Role) => {
    try {
      if (editingRole) {
        // 更新现有角色
  ...
  }, [editingRole, fetchRoles, pagination.current, pagination.pageSize, searchText]);

  // 处理分配权限
  const handleAssignPermission = useCallback(async (roleId: string, permissions: any[]) => {
    try {
  ...
  // 显示分配权限模态框
  const showPermissionModal = useCallback((role: Role) => {
    setEditingRole(role);
  ...
  return (
    <Card style={styles.card}>
      <Title level={4} style={styles.header}>角色管理</Title>
      <Divider />
