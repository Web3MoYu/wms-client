...
export interface OrderOut {
  ...
  approver: string; // 审核人
  inspector: string | null; // 质检员
  expectedTime: Date | null; // 预计到达时间
  actualTime: Date | null; // 实际到达时间
  totalAmount: number; // 总金额
  totalQuantity: number; // 总数量
  status: number | null; // 状态：0-待审核，1-审批通过，2-入库中/出库中，3-已完成，-1-已取消，-2-审批拒绝
  qualityStatus: number | null; // 质检状态：质检状态：0-未质检，1-质检通过，2-质检不通过，3-部分异常
...
export interface OrderOutItem {
  id: string | null; // 明细ID
  orderId: string | null; // 出库订单ID
  productId: string; // 产品ID
  productName: string | null; // 产品名称
  productCode: string | null; // 产品编码
  expectedQuantity: number; // 预期数量
  actualQuantity: number | null; // 实际数量
  price: number; // 单价
  amount: number; // 金额
